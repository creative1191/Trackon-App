...
class TrackonApp:
    def __init__(self, root):
        ...
        self.colors = {
            "bg": "#F5F7FA", "card_bg": "#FFFFFF", "primary": "#1A237E",
            "text_dark": "#263238", "text_light": "#90A4AE",
            "prime_color": "#C2185B", "std_color": "#2E7D32"
        }
    ...
    def create_widgets(self):
        ...
        try:
            ...
            logo_label.config(text="TRACKON", font=("Segoe UI", 28, "bold"), fg=self.colors["primary"], bg=self.colors["card_bg"])
            
        footer_text = tk.Label(header_frame, text="S W I F T . S A F E . S U R E", font=("Segoe UI", 9, "bold"), bg=self.colors["card_bg"], fg=self.colors["text_light"])
        ...
        lbl_company = tk.Label(card, text="Select Company", font=("Segoe UI", 10, "bold"), bg=self.colors["card_bg"], fg="#546E7A", anchor="w")
        ...
        self.btn_prime = tk.Radiobutton(toggle_frame, text="PRIME", variable=self.company, value="prime", indicatoron=0, width=12, bg="#FCE4EC", fg="#880E4F", selectcolor="#E91E63", activebackground="#F48FB1", font=("Segoe UI", 10, "bold"), bd=0, relief="flat", cursor="hand2")
        self.btn_prime.pack(side="left", padx=2)

        self.btn_std = tk.Radiobutton(toggle_frame, text="STANDARD", variable=self.company, value="standard", indicatoron=0, width=12, bg="#E8F5E9", fg="#1B5E20", selectcolor="#00C853", activebackground="#A5D6A7", font=("Segoe UI", 10, "bold"), bd=0, relief="flat", cursor="hand2")
        ...
        self.entry_wt = tk.Entry(card, font=("Segoe UI", 14, "bold"), justify="center", bg="#F5F7FA", relief="flat", bd=1)
        self.entry_wt.pack(fill="x", padx=25, pady=(2, 10))
        self.entry_wt.bind("<Return>", lambda e: self.calculate())

        calc_btn = tk.Button(card, text="CALCULATE RATE", command=self.calculate, bg=self.colors["primary"], fg="white", font=("Segoe UI", 13, "bold"), activebackground="#0D47A1", relief="flat", cursor="hand2", height=2)
        calc_btn.pack(fill="x", padx=25, pady=10)

        res_frame = tk.Frame(card, bg="#ECEFF1", relief="flat")
        res_frame.pack(fill="x", padx=25, pady=15)
        self.lbl_result_label = tk.Label(res_frame, text="TOTAL AMOUNT", font=("Segoe UI", 8, "bold"), bg="#ECEFF1", fg="#546E7A")
        self.lbl_result_label.pack(pady=(8, 0))
        self.lbl_amt = tk.Label(res_frame, text="Rs 0", font=("Segoe UI", 38, "bold"), bg="#ECEFF1", fg=self.colors["primary"])
        self.lbl_amt.pack()
        self.lbl_detail = tk.Label(res_frame, text="Select options & enter weight", font=("Segoe UI", 9), bg="#ECEFF1", fg="#78909C")
        self.lbl_detail.pack(pady=(0, 8))
        
        footer = tk.Label(self.root, text="2026 Trackon Courier System", font=("Segoe UI", 8), bg=self.colors["bg"], fg="#B0BEC5")
        footer.pack(pady=5)

    def add_input_row(self, parent, text):
        lbl = tk.Label(parent, text=text, font=("Segoe UI", 10, "bold"), bg=parent.cget("bg"), fg="#546E7A", anchor="w")
        ...
        try:
            if comp == 'prime':
                if svc == 'dox':
                    ...
                    detail = f"DOX: {slabs} slab(s) x Rs{rates[zone]}"
                ...
                if svc == 'dox':
                    ...
                    amt = rate * cw
                    detail = f"Surface: Rs{rate}/kg ({slab_info}) x {cw}kg"
                elif svc == 'smart':
                    cw = math.ceil(w)
                    rates = {'mp': 81, 'roi': 104, 'ne': 126}
                    amt = rates[zone] * cw
                    detail = f"Smart: Rs{rates[zone]}/kg x {cw}kg"
    ...
    def update_result(self, amount, detail_text, color):
        self.lbl_amt.config(text=f"Rs{amount}", fg=color)
